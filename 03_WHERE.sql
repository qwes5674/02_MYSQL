...
/*
tbl_menu 테이블에 있는 컬럼
menu_name, menu_price, orderable_status 를 조회할 건데
단, menu_price 가 13000 인 데이터만 조사해줘
*/
...
-- select에 담지 않아도, 다른 칼럼의 속성으로 제한 할 수 있다.
...
-- ColumnA != B 컬럼에 있는 내용 중 B와 같지 않는 데이터만 보여주기
-- ColumnA <> B 컬럼에 있는 내용 중 B와 같지 않는 데이터만 보여주기
...
-- 둘중에 하나라도 false 면 false, 둘 다 true 여야지 true
/*
true = 1; // false = 0
null : 값이 없다.
*/
...
select 1 or 0 and 0;
-- 1 or 0 이 먼저 연산되므로 두번째 연사나 1 AND 0 => 0
select (1 or 0) and 0;
...
from
    tbl_menu
where
    menu_price >= 10000 and -- 메뉴가격이 10000원 이상이고
    menu_price <= 25000 -- 메뉴가격이 25000 이하
...
from
    tbl_menu
where
    menu_price between 10000 and 25000 -- 메뉴 가격이 10000 이상 25000 이하
